...
class ObjectDetector:
    ...
    def download_and_resize_image(url, new_width=256, new_height=256, display=False):
        ...
        if display:
            ObjectDetector.display_image(pil_image)
        return filename
    ...
    def draw_boxes(image, boxes, class_names, scores, max_boxes=10, min_score=0.1):
        ...
        for i in range(min(boxes.shape[0], max_boxes)):
            if scores[i] >= min_score:
                ...
                ObjectDetector.draw_bounding_box_on_image(
                    image_pil,
                    ymin,
                    xmin,
                    ymax,
    ...
    def run_detector(detector, path):
        img = ObjectDetector.load_img(path)
        ...
        image_with_boxes = ObjectDetector.draw_boxes(
            img.numpy(),
    ...
    def save_processed_image(image_with_boxes):
        _, filename = tempfile.mkstemp(suffix=".jpg")
        im = Image.fromarray(image_with_boxes)
        im.save(filename, format="JPEG", quality=90)
...
class GoogleSearch:
    def object_name_finder(filePath):
        ...
        browser.quit()
        result = soup.find("a", attrs={"class": "fKDtNb"}).text
...
class Menu(QMainWindow):
    def __init__(self, path):
        super().__init__()
        self.setWindowTitle("Detected Objects")
        self.path = path
